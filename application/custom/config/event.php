    ...
    if ($type == 'controller') {
        ...
        $this->event->on('controller/member/joinInputBasic',
            function ($e) {
                ...
                if (!empty($params)) {
                    sendMessage('member_reg_new', $params['user']['email'], $params['user']['pcs'],
                        [
            ...
            function ($e) {
                ...
                if (!empty($params['oid']) && $params['sendCancelComplete']) {
                    ...
                    if ($cancelInfo['order']['status'] == ORDER_STATUS_CANCEL_COMPLETE) {
                        sendMessage('order_cancel_new', $cancelInfo['order']['bmail'], $cancelInfo['order']['bmobile'], $cancelInfo['order']);
                    }
                }
            });
        ...
        $this->event->on('withdrowMemberSendEmail',
            function ($e) {
                $params = $e->getParams();
                sendMessage('member_exit_new', $params['mem_mail'], $params['mem_mobile'], $params);
            });
        ...
        $this->event->on('payment',
            function ($e) {
                ...
                if (!empty($params['oid'])) {
                    ...
                    sendMessage(
                        ($orderData['paymentInfo']['payment'][0]['method'] == ORDER_METHOD_VBANK && $orderData['order']['status'] == ORDER_STATUS_INCOM_READY
                            ? 'order_sucess_vbank_new' : 'order_sucess_new')
        ...
        //if(strpos($_SERVER['REQUEST_URI'],'shop/goodsView')){
