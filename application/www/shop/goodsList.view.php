...
$cateName = $productModel->getCategoryName($cid);

$rankInfo = $productModel->getCategoryRanking(5, $cid);

/*if(is_mobile()){
    $bannerInfo = $displayModel->getDisplayBannerGroup('44','banner_ix',$cid);
}else{
...
$subBannerInfo = $displayModel->getDisplayBannerGroup('26','banner_ix',$cid);
...
for($i=0; $i<count($cateData); $i++) {
	...
	*/
    ...
    if($cateDepth == 0) {
		...
		if(substr($cateData[$i]['cid'],0,6) == substr($cid,0,6)) {
			if($cateData[$i]['depth'] == 2){;
				$cateArrList['subCate'][$cate['vlevel3']-1] = $cateData[$i];
			}
		}
    }else if($cateDepth == 2) {
		if(substr($cateData[$i]['cid'],0,6) == substr($cid,0,6)) {
...
$view->assign('chkMenu', "goodsLists");
...
$view->assign('subBannerInfo', $subBannerInfo);

#베스트 5
$view->assign('rankInfo', $rankInfo);
