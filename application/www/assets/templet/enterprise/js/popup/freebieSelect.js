...
var devGiftSelect = {
    ...
    _eventBind: function(){
                        ...
                        if (response.result == 'success') {
                            if(response.data.giftStockBool == false){
                                ...
                                $(giftData).each(function(i,v){
                                    giftHtml +='<div class="product-gift__box devGiftListByOrder">';
                                    giftHtml +='<figure class="product-gift__thumb">';
                                    ...
                                    giftHtml +='</figure>';
                                    giftHtml +='<div class="product-gift__info">';
                                    giftHtml +='<p class="product-gift__info__pname">'+v.pname+' / '+v.cnt+'개</p>';
                                    giftHtml +='</div>';
                                    giftHtml +='</div>';
                                });
        ...
        $('.devNoChoice').on('click',function(){
            ...
            giftHtml +='<div class="product-gift__box devGiftListByOrder">';
            giftHtml +='<figure class="product-gift__thumb">';
            giftHtml +='<img src="'+imgSrc+'/images/common/icon_no-freebie.png" alt="" data-freegift_condition="'+freegift_condition+'">';
            giftHtml +='</figure>';
            giftHtml +='<div class="product-gift__info">';
            giftHtml +='<p class="product-gift__info__pname">'+common.lang.get('gift.not.select.text')+'</p>';
            giftHtml +='</div>';
            giftHtml +='</div>';
    ...
    _changeCount: function (type, obj) {//넘겨받은 obj로 +, - 이벤트 수행
        ...
        //수량 변하면 클래스 추가 (프론트)
        if (data.cnt > 0) {
            $(obj).parents(".devGiftList").find(".freebie-select__box__thumb")
                .addClass("freebie-select__box__thumb--checked");
        } else {
            $(obj).parents(".devGiftList").find(".freebie-select__box__thumb")
                .removeClass("freebie-select__box__thumb--checked");
        }
