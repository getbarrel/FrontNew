...
$(function () {
    if(common.langType != 'korean') {
        ...
        var cmtTpl = myReviewList.compileTpl('#devMyReviewCmt');

        // setContent 재정의
        myReviewList.setContent = function (list, paging) {
            this.removeContent();
            if (list.length > 0) {
                for (var i = 0; i < list.length; i++) {
                    ...
                    console.log(row.cmt)
                    if (row.cmt) {
                        if (row.cmt.cmt_contents != '') {
                            $(this.container).append(cmtTpl(row.cmt));
                        }
        ...
        $('#devMyReviewContent').on('click', '.txt-area', function () {
            common.util.popup('/shop/goodsReview?mode=read&bbsIx=' + $(this).data('bbsix'), 720, 870, '상품 후기 내역');
        });
