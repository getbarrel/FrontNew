...
$('#devBbsRegSubmit').click(function (e) {
    ...
    // if($('#devBbsDiv').val() == ''){
    //     common.noti.alert(common.lang.get('write.customer.bbsdev.fail'));
    //     return false;
    // }
...
$('#devBtnOrderQuery').click(function (e) {
    var modal_title = '주문번호 조회';

    if(common.langType == 'english') {
        modal_title = 'Search order';
    }
    common.util.modal.open('ajax', modal_title, '/popup/orderList', '', '', 'order_search');
});
...
$('#devModalContent').on('click', '.btn-xs', function () {
    ...
    $("#orderOid").html($(this).attr('devselectoid'));
    $("#orderDay").html($(this).attr('devSelectdate'));
    $("#orderTitle").html($(this).attr('devSelecttitle'));
	$("#orderimg").attr("src", $(this).attr('devSelectimg'));
	$("#orderPrice").html($(this).attr('devSelectprice'));
    $("#devOrderCheckList").show();
    $('.popup-mask').hide();
    $('.popup-layout').hide();
