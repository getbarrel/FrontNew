...
var devCouponPopObj = {
        ...
        if ($select.length > 0) {
            ...
            if (data.paymentMethod != '') {
                ...
                $.each(data.paymentMethod.split('|'), function (key, method) {
                    var text = $("label[for='pay-method-" + method + "'").text();
                    if (text) {
    ...
    run: function () {
        ...
        $('#devApplyCouponButton').click(function () {
            ...
            //결제 수단 수정
            if (self.selectPaymentMethod != '') {
                $('input[name=devPaymentMethod]').each(function () {
                    $(this).closest('li').hide();
                });
                $.each(self.selectPaymentMethod.split('|'), function (key, method) {
                    $('input[name=devPaymentMethod][value=' + method + ']').closest('li').show();
                });
                $('input[name=devPaymentMethod]:not(:hidden):eq(0)').trigger('click');
            } else {
                $('input[name=devPaymentMethod]').each(function () {
