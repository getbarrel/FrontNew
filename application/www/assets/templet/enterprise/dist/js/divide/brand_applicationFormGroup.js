 ...
 */
const brand_applicationFormGroup = () => {
    ...
    const terms_check = () => {
        ...
        $document.on("click", "#all_terms_check", function() {
            if ($(this).is(':checked')) {
                $(".agree-area input[type=checkbox]").prop('checked', true);
            } else {
                $('.agree-area input[type=checkbox]').prop('checked', false);
            }
        });
    ...
    const team_number = () => {
        ...
        const addBtnDisable = () => {
            ...
            const _min = 0;
            const _sheetNum = $(".entry-form__sheet").length;
        ...
        $document.on("keyup", ".team-number .team-number__input", function () {
            ...
            if(_num>=1) {
                // 개수 노출
                $entryForm.addClass("entry-form--show");
                ...
                // 비노출 상태
                $entryForm.removeClass("entry-form--show");
            }
        });

        // 입력폼 삭제 버튼 클릭 이벤트
        // $document.on("click", ".entry-form__sheet--delete", function(e){
        //     e.preventDefault();
        //
        //     const $sheet = $(this).parents(".entry-form__sheet");
    ...
    const pw_regexp = () => {
        ...
        $document.on("keyup", ".inputs__password input[type=text]", function () {
    ...
    const applicationFormGroup_init = () => {
        terms_check();
        name_input();
        click_zipcode();
        team_number();
    ...
    applicationFormGroup_init();

}

export default brand_applicationFormGroup;