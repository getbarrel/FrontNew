...
$(function () {
    ...
    if (isAjaxList == 'undefined' || isAjaxList != 'N') {
        ...
        $('#devPageWrap').on('click', '.devPageBtnCls', function () { // 페이징 버튼 이벤트 설정
            self.bbsList.getPage($(this).data('page'));
        });
    }
    ...
    $("button[id^='devBbsFileDeleteButton']").click(function (e) {
        ...
        if (common.noti.confirm(common.lang.get('write.customer.file.confirm.delete'))) {
            ...
            var $targetBox = $(this).closest(".write-upload-box");
            deleteConfirmOk($targetBox, sNo['1']);
        }else{
            return false;
        }
    });

    var deleteConfirmOk = function ($targetBox, no) {
        $('#devBbsFileText' + no).val('');
        $targetBox.find(".write-upload-name").val("");
    };
    ...
    $("input[id^='devBbsFile']").change(function (e) {
        ...
        if(this.value != '' && rs == -1){
            ...
            var filename_split = $(this)[0].files[0].name;
            if (fname != "" && fname != "undefined" ) {
                ...
                $(this).closest(".write-upload-box").find(".write-upload-name").val(filename_split);
            } else {
