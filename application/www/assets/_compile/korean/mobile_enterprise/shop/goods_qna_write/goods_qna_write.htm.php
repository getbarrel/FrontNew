<?php /* Template_ 2.2.8 2023/07/18 10:19:58 /home/barrel-qa/application/www.bak/assets/mobile_templet/mobile_enterprise/shop/goods_qna_write/goods_qna_write.htm 000013324 */ 
...
<?php }}?>
                ...
                <div class="write-form__title">
                    <input type="text" name="subject" value="<?php echo $TPL_VAR["bbs_subject"]?>" id="devQnaSubject" title="문의 제목" placeholder="문의 제목을 입력해 주세요.">
                </div>
                <div class="write-form__email">
                    <input type="text" name="emailId" id="devEmailId" title="이메일" value="<?php echo $TPL_VAR["emailId"]?>" class="write-form__email--id">
                    <span class="hyphen_2">@</span>
                    <input type="text" name="emailHost" id="devEmailHost" title="이메일호스트" value="<?php echo $TPL_VAR["emailHost"]?>" class="write-form__email--host">
...
<?php }?>
                ...
                <div class="write-form__send-email">
                    <p class="write-form__send-email__desc">답변 여부를 메일로 받으시겠습니까?</p>
                    <div class="write-form__send-email__radio">
                        <label><input type="radio" name="bbs_email_return" value="1" <?php if($TPL_VAR["bbs_email_return"]== 1){?> checked <?php }?> ><span>예</span></label>
                        <label><input type="radio" name="bbs_email_return" value="0" <?php if($TPL_VAR["bbs_email_return"]== 0||$TPL_VAR["bbs_email_return"]==''){?> checked <?php }?>><span>아니오</span></label>
                    </div>
                </div>
...
<?php }?>
                    <textarea name="contents" id="devQnaContents" title="문의 내용"><?php echo $TPL_VAR["bbs_contents"]?></textarea>
                    <!--<div>문의전유의사항</div>-->
                </div>
                <div class="write-form__secret">
                    <p class="write-form__secret__desc">비밀글 설정을 하시겠습니까?</p>
                    <div class="write-form__secret__radio">
                        <label><input type="radio" name="isHidden" value="1" checked><span>예</span></label>
                        <label><input type="radio" name="isHidden" value="0"><span>아니오</span></label>
                    </div>
                </div>
                <ul class="write-form__notice">
                    <li class="write-form__notice__desc">- 전화번호, 이메일, 배송지 주소, 환불계좌 정보 등 개인정보가 포함된 게시글은 개인정보 도용의 <br>위험이 있으니 비밀글로 문의해 주시기 바랍니다.</li>

                    <li class="write-form__notice__desc">- 상품과 관계없는 내용, 비방, 욕설, 광고, 도배 등의 게시물은 예고 없이 삭제될 수 있습니다.</li>
                </ul>
                <div class="write-form__submit">
                    <button class="write-form__submit__btn" id="devSubmitBtn">등록</button>
                </div>
            </div>
        </div>
    </form>
