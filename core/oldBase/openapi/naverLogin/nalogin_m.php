...
class nalogin_m{
    ...
    public function isInnaId($id){
        $sql = "SELECT * FROM common_user WHERE na_id = '".$id."'";
        $this->db->query($sql);
     ...
     */
    public function naLoginProcess($data){
        ...
        $sql = "SELECT 
				...
				WHERE 
                    cu.na_id= '".$data["userID"]."' and cu.mem_type in ('M','F','C','S') 
    ...
    public function idDupCheck($id){
        $sql = "SELECT code FROM common_user where na_id = '".$id."' LIMIT 1";
		$this->db->query($sql);
