...
class fblogin_m{
     ...
     * @return {boolean} T/F
     */
    public function isInFbId($id){
        $sql = "SELECT * FROM common_user WHERE fb_id = '".$id."'";
        $this->db->query($sql);
        //syslog(LOG_INFO,"isInFbId : ".$sql);
        if($this->db->total){
            return true;
        }else{
            return false;
     ...
     */
    public function fbLoginProcess($id){
        ...
        $sql = "SELECT 
				...
				WHERE 
                    cu.fb_id= '".$id."' and cu.mem_type in ('M','F','C','S') 
    ...
    public function idDupCheck($id){
        $sql = "SELECT code FROM common_user where fb_id = '".$id."' LIMIT 1";
		$this->db->query($sql);
    ...
    public function insertUserInfo($data){
        ...
        if($this->db->query($sql)){
            $sql = "INSERT INTO
                    ...
                    SET
                        code = '".$code."',
                        name = HEX(AES_ENCRYPT('".$data["name"]."','".$this->db->ase_encrypt_key."')),
                        date = NOW(),
                        gp_ix = '1',
						mail = HEX(AES_ENCRYPT('".$data["email"]."','".$this->db->ase_encrypt_key."')),
                        sex_div = '".$sex_div."',
                        add_etc1 = '".$add_etc1."',
