...
if($act){
	...
	if($db->total){
		...
		if($block_ips != ""){
			...
			if($ip_check_bool){
				echo "<Script>alert('차단된 IP 주소 입니다.  확인후 다시 시도해주세요 . 계속해서 문제가 될 경우 고객센타로 문의주시기 바랍니다.');document.location.href='?mode=list&cf_ix=$cf_ix&cfbm_ix=$cfbm_ix&bbs_div=$bbs_div';</Script>";
				exit;
			}
		}
		...
		if($spam_words != ""){
			...
			if($spam_check_bool){
				echo "<Script>alert('스팸문자가 포함되어 글 등록이 차단되었습니다. 확인후 다시 시도해주세요 . 계속해서 문제가 될 경우 고객센타로 문의주시기 바랍니다.');document.location.href='?mode=list&cf_ix=$cf_ix&cfbm_ix=$cfbm_ix&bbs_div=$bbs_div';</Script>";
				exit;
			}
		}
	}
...
if($act == "insert"){
	...
	$db->query("select IFNULL(max(bbs_ix),0) as bbs_ix from cafe_bbs  ");
	if($db->total){
		$db->fetch();
	...
	/*$sql = "select IFNULL(max(bbs_ix_step )+1,1) as bbs_ix_step from cafe_bbs where bbs_top_ix =  ". $bbs_parent_ix;
	$db->query($sql);
	if($db->total){
		$db->fetch();
	...
	$sql = "select IFNULL(max(bbs_ix_level )+1,0) as bbs_ix_level from cafe_bbs where bbs_top_ix = ". $bbs_parent_ix ." and bbs_ix_level = ".$bbs_ix_level;
	$db->query($sql);
	if($db->total){
		$db->fetch();
	...
	$sql = "insert into cafe_bbs (bbs_ix,cfbm_ix, bbs_div,sub_bbs_div, mem_ix,bbs_subject,bbs_name,bbs_pass,bbs_email,bbs_contents,bbs_top_ix, bbs_ix_level, bbs_ix_step, bbs_hidden, bbs_file_1,bbs_file_2,bbs_file_3, bbs_etc1,bbs_etc2,bbs_etc3,bbs_etc4,bbs_etc5,is_notice, is_html, ip_addr, regdate)
			values
			('$bbs_ix','$cfbm_ix','$bbs_div','$sub_bbs_div','".$bbs_user_code."','$bbs_subject','$bbs_name','$bbs_pass','$bbs_email','$bbs_contents','$bbs_ix','$bbs_ix_level','$bbs_ix_step','$bbs_hidden','$bbs_file_1_name','$bbs_file_2_name','$bbs_file_3_name','$bbs_etc1','$bbs_etc2','$bbs_etc3','$bbs_etc4','$bbs_etc5','$is_notice','$is_html','".$_SERVER["REMOTE_ADDR"]."',NOW())";

//	$sql = "insert into cafe_bbs (bbs_ix,bbs_div,mem_ix,bbs_subject,bbs_name,bbs_pass,bbs_email,bbs_contents,bbs_hidden, bbs_file_1,bbs_file_2,bbs_file_3, bbs_etc1,bbs_etc2,bbs_etc3,bbs_etc4,bbs_etc5, regdate)
//			values
//			('','$bbs_div','".$bbs_user_code."','$bbs_subject','$bbs_name','$bbs_pass','$bbs_email','$bbs_contents','$bbs_hidden','$bbs_file_1_name','$bbs_file_2_name','$bbs_file_3_name','$bbs_etc1','$bbs_etc2','$bbs_etc3','$bbs_etc4','$bbs_etc5',NOW())";
	...
	$db->query($sql);
	//$db->query("Select bbs_ix from cafe_bbs where bbs_ix = LAST_INSERT_ID()");
	$db->query("Select bbs_ix from cafe_bbs where bbs_ix = '$bbs_ix' ");

	//echo "Select bbs_ix from cafe_bbs where bbs_ix = $bbs_ix ";
...
//	exit;
	...
	$db->query("UPDATE cafe_bbs SET bbs_contents = '$data_text' WHERE bbs_ix='$bbs_ix'");
	*/

	//exit;
	echo "<Script>top.document.location.href='?mode=list&cf_ix=$cf_ix&cfbm_ix=$cfbm_ix&bbs_div=$bbs_div';</Script>";
}
...
if($act == "update"){
	...
	$db->query("select bbs_file_1,bbs_file_2,bbs_file_3 from cafe_bbs where bbs_ix = '".$bbs_ix."'");
	$db->fetch();
//echo "select bbs_file_1,bbs_file_2,bbs_file_3 from cafe_bbs where bbs_ix = '".$bbs_ix."'";
//exit;
	if ($bbs_file_1_size > 0){
		if($db->dt[bbs_file_1] != ""){
	...
	if ($bbs_file_2_size > 0){
		...
		/*썸네일 이미지 생성 시작*/
		$image_info = getimagesize ($path."/".iconv("utf-8","CP949",$bbs_file_2_name));
		$image_type = substr($image_info['mime'],-3);
	}
	...
	if ($bbs_file_3_size > 0){
		...
		/*썸네일 이미지 생성 시작*/
		$image_info = getimagesize ($path."/".iconv("utf-8","CP949",$bbs_file_3_name));
		$image_type = substr($image_info['mime'],-3);
	}
	/*썸네일 이미지 생성 시작*/
	/*$image_info = getimagesize ($path."/".iconv("utf-8","CP949",$bbs_file_1_name));
	...
	$sql ="	update cafe_bbs set
			bbs_subject='".trim($bbs_subject)."',cfbm_ix='$cfbm_ix',bbs_div='$bbs_div',sub_bbs_div='$sub_bbs_div',bbs_name='$bbs_name',bbs_pass='$bbs_pass',bbs_email='$bbs_email',bbs_contents='$bbs_contents', bbs_hidden ='$bbs_hidden',
	...
	if($bbs_mode == "slide"){
		echo "<Script>document.location.href='?mode=list&cf_ix=$cf_ix&cfbm_ix=$cfbm_ix&page=$page&bbs_ix=$bbs_ix&bbs_div=$bbs_div';</Script>";
	}else{
		echo "<Script>top.document.location.href='?mode=read&cf_ix=$cf_ix&cfbm_ix=$cfbm_ix&article_no=$article_no&bbs_ix=$bbs_ix&bbs_div=$bbs_div';</Script>";
	}
	exit;
...
if($act == "delete"){
	...
	if($admin_mode || ($_SESSION["user"]["code"] == $msb->bbs_config->dt[cafe_owner])){
		$db->query("Select * from cafe_bbs where bbs_ix='$bbs_ix' ");
			...
			$sql ="	delete from cafe_bbs where bbs_ix='$bbs_ix'";
			$db->query($sql);
			$sql ="	delete from cafe_bbs where bbs_top_ix=$bbs_ix ";
			$db->query($sql);
			$sql ="delete from cafe_bbs_comment where bbs_ix='$bbs_ix'";
			$db->query($sql);

			echo "<Script>document.location.href='?mode=list&cf_ix=$cf_ix&cfbm_ix=$cfbm_ix&bbs_div=$bbs_div';</Script>";

	}else if(($bbs_user_code != "") && !$admin_mode && ($_SESSION["user"]["code"] != $msb->bbs_config->dt[cafe_owner])){
		$db->query("Select * from cafe_bbs where mem_ix = '".$bbs_user_code."' and bbs_ix='$bbs_ix' ");
		...
		if($db->total){
			...
			$sql ="	delete from cafe_bbs where bbs_ix='$bbs_ix'  and mem_ix = '".$bbs_user_code."' ";
			$db->query($sql);
			$sql ="	delete from cafe_bbs where bbs_top_ix=$bbs_ix ";
			$db->query($sql);
			$sql ="delete from cafe_bbs_comment where bbs_ix='$bbs_ix'";
		...
		$db->query("Select * from cafe_bbs where bbs_pass = '$bbs_pass' and bbs_ix='$bbs_ix' ");
...
if($act == "response"){
	...
	$sql = "select IFNULL(max(bbs_ix_step )+1,0) as bbs_ix_step from cafe_bbs where cfbm_ix = '$cfbm_ix' and bbs_top_ix =  '". $bbs_top_ix."' ";
	...
	$sql = "select IFNULL(max(bbs_ix_level )+1,0) as bbs_ix_level from cafe_bbs where cfbm_ix = '$cfbm_ix' and bbs_top_ix = ". $bbs_parent_ix ." and bbs_ix_level = ".$bbs_ix_level;
	$db->query($sql);
	if($db->total){
		$db->fetch();
		$bbs_ix_level = $db->dt[bbs_ix_level];
	...
	$sql = "insert into cafe_bbs (bbs_ix,cfbm_ix,bbs_div,mem_ix,bbs_subject,bbs_name,bbs_pass,bbs_email,bbs_contents,bbs_top_ix, bbs_ix_level, bbs_ix_step, bbs_hidden, bbs_file_1,bbs_file_2,bbs_file_3, bbs_etc1,bbs_etc2,bbs_etc3,bbs_etc4,bbs_etc5, is_html, ip_addr, regdate)
			values
			('$bbs_ix','$cfbm_ix','$bbs_div','".$bbs_user_code."','$bbs_subject','$bbs_name','$bbs_pass','$bbs_email','$bbs_contents','$bbs_parent_ix','$bbs_ix_level','$bbs_ix_step','$bbs_hidden','$bbs_file_1_name','$bbs_file_2_name','$bbs_file_3_name','$bbs_etc1','$bbs_etc2','$bbs_etc3','$bbs_etc4','$bbs_etc5','$is_html','".$_SERVER["REMOTE_ADDR"]."', NOW())";
...
if($act == "comment_insert"){
	...
	$sql = "insert into cafe_bbs_comment
	...
	$db->query("update cafe_bbs set bbs_re_cnt = bbs_re_cnt + 1 where bbs_ix ='$bbs_ix'");
	if($_POST["bbs_pass"]){
		echo "<Script>document.location.href='?mode=read&cf_ix=$cf_ix&cfbm_ix=$cfbm_ix&article_no=$article_no&bbs_ix=$bbs_ix&bbs_div=$bbs_div&bbs_pass=".$_POST["bbs_pass"]."';</Script>";
	}else{
		echo "<Script>document.location.href='?mode=read&cf_ix=$cf_ix&cfbm_ix=$cfbm_ix&article_no=$article_no&bbs_ix=$bbs_ix&bbs_div=$bbs_div';</Script>";
	}
}
...
if($act == "comment_delete"){
	if($admin_mode){
		$sql ="select * from cafe_bbs_comment where cmt_ix = '$cmt_ix' and bbs_ix='$bbs_ix'";
	}else{
		if($cmt_pass != ""){
			$sql ="select * from cafe_bbs_comment where cmt_ix = '$cmt_ix' and bbs_ix='$bbs_ix' and cmt_pass ='$cmt_pass' ";
		}else{
			$sql ="select * from cafe_bbs_comment where cmt_ix = '$cmt_ix' and bbs_ix='$bbs_ix' and mem_ix ='".$bbs_user_code."' ";
		}
	}
	//echo $sql;
	$db->query($sql);
	...
	if($db->total){
		$sql ="	delete from cafe_bbs_comment where cmt_ix = '$cmt_ix' and bbs_ix='$bbs_ix' ";
	//	echo $sql;
		$db->query($sql);
		$db->query("update cafe_bbs set bbs_re_cnt = bbs_re_cnt - 1 where bbs_ix ='$bbs_ix'");
...
if($act == "pass_check" || $act == "pass_check_for_read"){
	if($bbs_user_code != "" && $bbs_pass == ""){  //로그인 이고 자기글일때 바로 접근할때만
		...
		$sql ="select * from cafe_bbs where bbs_ix='$bbs_ix' and bbs_pass ='$bbs_pass' ";
	//	echo $sql;
		$db->query($sql);
	}
	...
	if($db->total){
		if($act == "pass_check_for_read"){
			echo "<Script>document.location.href='?mode=read&cf_ix=$cf_ix&cfbm_ix=$cfbm_ix&bbs_ix=$bbs_ix&bbs_pass=$bbs_pass';</Script>";
		}else{
			echo "<Script>document.location.href='?mode=modify&cf_ix=$cf_ix&cfbm_ix=$cfbm_ix&bbs_ix=$bbs_ix&article_no=$article_no&page=$page&bbs_pass=$bbs_pass';</Script>";
		}
	}else{
...
if($act == "faq_update"){
	...
	echo "<Script>document.location.href='?cf_ix=$cf_ix&cfbm_ix=$cfbm_ix&mode=list';</Script>";
	exit;
}
...
if($act == "select_delete"){
	...
	if($cnt > 0){
		for($i=0;$i<$cnt;$i++){
			...
			$sql ="	delete from cafe_bbs where bbs_ix='".$bbs_ix[$i]."' ";

			$db->query($sql);

			$sql ="delete from cafe_bbs_comment where bbs_ix='".$bbs_ix[$i]."'";
			$db->query($sql);
		}
	}
	echo "<Script>document.location.href='?mode=list&cf_ix=$cf_ix&cfbm_ix=$cfbm_ix&bbs_div=$bbs_div';</Script>";
