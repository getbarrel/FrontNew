...
if($PGIP == "211.219.96.165" || $PGIP == "118.129.210.25" || $PGIP == "183.109.71.153")	//PG에서 보냈는지 IP로 체크
  {
	...
	'***********************************************************************************/
	if( ($P_TYPE!="VBANK" && $P_STATUS == "00") || ($P_TYPE=="VBANK" && $P_STATUS == "02" ) ){
        ...
        $db_update = false;
		if($P_TYPE=="VBANK" && $P_STATUS == "02"){
            $db->query("select * from shop_order where oid='" . $P_OID . "' and status='IR' ");
            if ($db->total > 0) {
                $db->query("select * from shop_order_session where oid='" . $P_OID . "' ");
                $db->fetch();
                $_SESSION = unserialize(urldecode($db->dt['data']));
                @extract($_SESSION);
                $db_update = true;
            }
		}else{
		...
		if($db_update == true){
            $pg_com = $sattle_module = "inipay_standard";
            include $_SERVER['DOCUMENT_ROOT'] . '/shop/inipay_standard/payment_processing.php';
		}else{
            $pg_result_success = '1';
		}
	}else{
...
{
    $path = $_SERVER["DOCUMENT_ROOT"]."/shop/inipay_standard/log";
