...
class _Http {
    ...
    function setURL($url) {
        if (!$m = parse_url($url))
            return $this->setError("파싱이 불가능한 URL입니다.");
        if ($m['scheme'] != "http")
            return $this->setError("HTTP URL이 아닙니다.");
    ...
    function setCookie($key, $value = "") {
        ...
        if (substr($this->cookie, 0, 1) == ";")
            $this->cookie = substr($this->cookie, 2);
    }
    ...
    function postMethod() {
        ...
        if ($this->auth)
            $query .= "Authorization: Basic " . $this->auth . "\r\n";
        if ($this->referer)
            $query .= "Referer: " . $this->referer . "\r\n";
        if ($this->cookie)
    ...
    function getCookie($key = "") {
        if ($key) {
            $pattern = "/" . $key . "=([^;]+)/";
            if (preg_match($pattern, $this->_header, $ret))
                return $ret[1];
        } else {
