 ...
 * @property \ForbizAdminInfo $admin 관리자 정보
 */
class ForbizTest extends Forbiz
{
    protected $modelname;
    ...
    protected function view_footer()
    {
        ...
        if (!is_cli()) {
            $output[] = '</ol>';
            $output[] = '<br />';
            $output[] = sprintf('<strong>All tests completed in %s seconds</strong>',
    ...
    protected function _get_test_methods()
    {
        ...
        foreach ($methods as $method) {
            if (substr(strtolower($method), 0, 4) == 'test') {
                if (
                    (!empty($this->runTest) && !in_array($method, $this->runTest)) ||
                    (!empty($this->notRunTest) && in_array($method, $this->notRunTest))
                ) {
                    continue;
                } else {
                    $testMethods[] = $method;
    ...
    protected function debug($data)
    {
        ...
        $this->message .= is_cli() ? $message : ('<xmp>'.$message.'</xmp>');
    }

    protected function capture($class, $method, $params = [])
    {
        ob_start();
